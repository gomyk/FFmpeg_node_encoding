$(function(){
    ...
    socket.on('update file list',function(fileList){
        ...
        $.each(fileList,function(index,val){
            $('.video-list ul').append(
                '<li class="list-group-item" data-id="'+ val +'">' + val +'<a href="#"><i class="glyphicon glyphicon-floppy-save pull-right"></i></a></li>'
            );
        });
    });
    ...
    $('.video-list ul').on('click','li',function(e){
        ...
        window.location.replace("http://moonwon.gonetis.com:3000/download/"+val);
